...
NextEtappeButton::NextEtappeButton(Transform transform, bool active) : GameObject(transform, active) {
    ...
    buttons.insert(std::make_pair(Keys::NEXT_ETAPPE, Button{
            ...
            std::make_shared<SwitchSceneButtonBehavior>(Keys::NEXT_ETAPPE, audioSources.at(Keys::FALSE), true),
            true}));
}
