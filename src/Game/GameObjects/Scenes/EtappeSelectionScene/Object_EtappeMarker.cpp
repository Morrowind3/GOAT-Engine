...
Object_EtappeMarker::Object_EtappeMarker(const std::string& levelKey, int levelNumber, unsigned short height, Transform transform, bool active) : GameObject(transform, active) {
    ...
    buttons.insert(std::make_pair(Keys::TRUE, Button {
            ...
            Rectangle{{transform.position.x, transform.position.y}, 200, 200},
            std::make_shared<Behavior_SwitchSceneButton>(levelKey, audioSources.at(Keys::FALSE), true),
            false
    }));

    // Automatic configuration of this object
    behaviors.insert(std::make_pair(Keys::BEHAVIOR, std::make_shared<Behavior_EtappeMarker>(*this, levelNumber, true)));
}
