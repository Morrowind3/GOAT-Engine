...
Object_Player::Object_Player(Transform transform, bool active) : GameObject(transform, active) {
    ...
    // Render
    auto idleSprite = sprites.insert(std::make_pair(Keys::IDLE, Sprite_PlayerIdle{true})).first->second;
    ...
    auto walkingAnimator = animators.insert(std::make_pair(Keys::_, Animator_PlayerWalking{moveSprite1, moveSprite2, moveSprite3, true}));
    // Audio
